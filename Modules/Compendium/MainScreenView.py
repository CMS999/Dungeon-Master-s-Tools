...
""" class ddiItemRole(Qt.ItemDataRole):
	def __init__(self):
		super().__init__() """
...
class mainScreen(Ui_ScreenView):
	...
	def actionOpenInNewWindow(self, itemData: dict) -> None:
		...
		htmlRender.renderHTML(itemData[32])
	...
	def actionOpenInBrowser(self, itemData: dict) -> None:
		...
		browerFile.write(self.webViewer.formatHTML(itemData[32]))
		browerFile.close()
		...
		match ddiObject:
			...
			case Item():
				...
				if ddiObject.getIsMundane():
					self.model.setItem(index, self.columnsList.index("Column4"), QStandardItem("Yes"))
				else:
			...
			case Trap():
				...
				if ddiObject.getLevel().isnumeric():
					self.model.setItem(index, self.columnsList.index("Column4"), QStandardItem(ddiObject.getLevel()))
				else:
					self.model.setItem(index, self.columnsList.index("Column4"), QStandardItem("*"))

				if ddiObject.getXP() != -1:
					self.model.setItem(index, self.columnsList.index("Column5"), QStandardItem(str(ddiObject.getXP())))
				else:
					self.model.setItem(index, self.columnsList.index("Column5"), QStandardItem("*"))
				self.model.setItem(index, self.columnsList.index("Column6"), QStandardItem(ddiObject.getClasse()))


		self.model.setItem(index, self.columnsList.index("Column8"), QStandardItem(ddiObject.getSource()))
	...
	def textChanged(self, text: str) -> None:
		...
		self.ddiTable.selectionModel().selectionChanged.connect(lambda: self.webViewer.renderHTML(self.ddiTable.model().index(self.ddiTable.currentIndex().row(), 0).data(32)))
